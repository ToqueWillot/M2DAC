require 'torch'
local CharLMMinibatchLoader=require 'CharLMMinibatchLoader'

cmd = torch.CmdLine()
cmd:text()
cmd:text('Convert data to torch format')
cmd:text()
cmd:text('Options')
cmd:option('-datadir','../data/perso','data source')
cmd:option('-vocab','vocab.t7','name of the char->int table to save')
cmd:option('-data','data.t7','name of the serialized torch ByteTensor to save')
cmd:text()

file = 'input.txt'
params = cmd:parse(arg)
print(params)
CharLMMinibatchLoader.text_to_tensor(params.datadir..'/'..file, params.datadir..'/'..params.vocab, params.datadir..'/'..params.data)

