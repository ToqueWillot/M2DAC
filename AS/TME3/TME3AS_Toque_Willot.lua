...
out_1 = g:forward(x)
out_2 = t:forward(out_1)
out_3 = h:forward(out_2)
loss = c:forward(out_3,y)
delta1 = c:backward(out_3,y)
delta2 = h:updateParameters(delta1) -- delta1 ou autre chose

]]--

Requ, Parent = torch.class('Requ', 'nn.Module')

function Requ:__init()
   parent.__init(self)
end

function Requ:updateOutput(x)
  local t = torch.Tensor(x:size(1))
  --[[
  for i=1,x:size(1) do
    if x[i]>0 do
      t[i] = x[i]*x[i]
...
function Requ:updateGradInput(input, gradOutput)
  local t = torh.Tensor(input:size(1))
  for i=1,input:size(1) do
    t[i] = gradOutput[i]*input[i]
  end
  self.gradInput = t
